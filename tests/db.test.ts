...
describe('Database', () => {
  ...
  it('should enforce button CHECK constraint', () => {
    ...
    const userStmt = db.prepare('INSERT INTO users (chzzk_id, chzzk_nickname) VALUES (?, ?)')
    userStmt.run('test_user', 'test_nick')
    const user = db.prepare('SELECT id FROM users WHERE chzzk_id = ?').get('test_user') as { id: number }

    // Valid button should succeed
    const validStmt = db.prepare('INSERT INTO dj_classes (user_id, button, dj_class) VALUES (?, ?, ?)')
    expect(() => validStmt.run(user.id, 6, 'TEST CLASS')).not.toThrow()

    // Invalid button should fail
    const invalidStmt = db.prepare('INSERT INTO dj_classes (user_id, button, dj_class) VALUES (?, ?, ?)')
