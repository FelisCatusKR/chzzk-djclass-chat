...
function getThreshold(rankName: string, rankLevel: string | null): number | null {
...
function setCachedDjClass(key: string, entry: Omit<CacheEntry, 'expiry'>): void {
      ...
      while (pendingQueueRef.current.length > 0) {
        ...
        if (cached) {
          ...
          try {
            ...
            if (pending.senderNickname) params.append('chzzkNickname', pending.senderNickname)
            ...
            if (!response.ok) {
              ...
              if (result.unlinked) {
                ...
                cacheEntry.rankShort = result.rankName ? (SHORT_NAMES[result.rankName] || result.rankName) : null
  ...
  return (
    ...
    >
      ...
      {connectionStatus === 'error' && messages.length === 0 && (
        <div className="flex items-center justify-center h-full text-red-400/50 text-sm">
          채팅 연결 실패
        </div>
      )}
          ...
          >
            ...
            {msg.rankShort && (
              <span
                ...
                style={{
                  background: getDjClassColor(msg.djClass?.replace(/^\d+B\s+/, '').replace(/\s+(I|II|III|IV|V|VI|VII|VIII|IX|X)$/i, '').trim() || 'BEGINNER'),
                  color: '#000',
              ...
              >
                {(() => {
                  ...
                  if (mode === 'threshold') {
                    const rankName = msg.djClass?.replace(/^\d+B\s+/, '').replace(/\s+(I|II|III|IV|V|VI|VII|VIII|IX|X)$/i, '').trim() || 'BEGINNER'
                    const levelMatch = msg.djClass?.match(/\s+(I|II|III|IV|V|VI|VII|VIII|IX|X)$/i)
                    const rankLevel = levelMatch ? levelMatch[1] : null
                    const threshold = getThreshold(rankName, rankLevel)
                    return threshold != null ? `${buttonPrefix} ${threshold}+` : `${buttonPrefix} ${msg.rankShort}`
                  }
            ...
            {msg.isTheory && (
              <span
                className="inline-block px-1 py-0.5 rounded text-xs font-bold mr-1 shadow-sm theory-badge"
              >
                이론치
              </span>
            )}
