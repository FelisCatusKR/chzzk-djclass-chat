...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    ).get(userId) as { token_encrypted: string; varchive_nickname: string } | undefined

    if (!tokenRow) {
      return NextResponse.json({ error: 'V-ARCHIVE not linked' }, { status: 400 })
    }
    ...
    if (!userInfo.success) {
      return NextResponse.json({ error: 'Invalid V-ARCHIVE token' }, { status: 400 })
    }

    // Update nickname if changed
    if (userInfo.nickname !== tokenRow.varchive_nickname) {
      db.prepare('UPDATE varchive_tokens SET varchive_nickname = ? WHERE user_id = ?')
    ...
    const userRow = db.prepare('SELECT chzzk_id, chzzk_nickname FROM users WHERE id = ?').get(userId) as
    ...
    const djClass = djClassData ? `${djClassData.button}B ${djClassData.djClass}` : '4B BEGINNER'
